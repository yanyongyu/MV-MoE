...
ATT_SIZE = int(os.getenv("ATT_SIZE", "4"))
FFN_SIZE = int(os.getenv("FFN_SIZE", "3072"))
...
with args.main_process_first(local=False, desc="loading model"):
    model = apply_adapter(
        ...
        ffn_adapter={"medical": FFN_SIZE},
        attn_adapter={"medical": ATT_SIZE},
    ).from_pretrained(
        MODEL_NAME,
        torch_dtype="auto",
        device_map="auto",
...
model = nested_freeze_tensor(
    model,
    exclude={
        "model.layers.*.mlp.medical_gate.*",
        "model.layers.*.mlp.medical_up.*",
        "model.layers.*.mlp.medical_down.*",
        "model.layers.*.self_attn.medical_q_proj.*",
        "model.layers.*.self_attn.medical_k_proj.*",
        "model.layers.*.self_attn.medical_v_proj.*",
        "model.layers.*.self_attn.medical_o_proj.*",
    },
)
