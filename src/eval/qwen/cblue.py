...
def evaluate(model_dir: str, dataset_type: Literal["dev", "test"]):
    if IS_DEEPSEEK:
        tokenizer = AutoTokenizer.from_pretrained("./data/DeepSeek-R1-Distill-Qwen-7B")
    else:
    ...
    for result in results:
        result["target"] = _remove_think(result["target"])
...
def evaluate_xlora(model_dir: str, dataset_type: Literal["dev", "test"]):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_dir)
    model = AutoPeftModelForCausalLM.from_pretrained(
        model_dir, torch_dtype="auto", device_map="auto", use_cache=False
    )
