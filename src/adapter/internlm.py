...
from src.model.internlm.config import InternLM3Config
from src.model.internlm.model import (
    InternLM3Model,
...
MT = TypeVar("MT", bound=type[InternLM3PreTrainedModel])
...
class InternLM3MLPAdapter(nn.Module):
    def __init__(self, config: Config):
        ...
        for name, size in config.ffn_adapter.items():
            setattr(
                self,
                f"{name}_gate",
                nn.Linear(config.hidden_size, size, bias=config.bias),
            )
            setattr(
                self,
                f"{name}_up",
            ...
            setattr(
                self,
                f"{name}_down",
                nn.Linear(size, config.hidden_size, bias=config.bias),
            )
...
class InternLM3AttentionAdapter(nn.Module):
    def __init__(self, config: Config, layer_idx: int):
        ...
        for name, num in config.attn_adapter.items():
            setattr(
                self,
                f"{name}_q_proj",
                nn.Linear(self.hidden_size, self.head_dim * num, bias=config.qkv_bias),
            )
            setattr(
                self,
                f"{name}_k_proj",
            ...
            setattr(
                self,
                f"{name}_o_proj",
                nn.Linear(self.head_dim * num, self.hidden_size, bias=config.bias),
            )
...
class InternLM3DecoderLayerAdapter(nn.Module):
    def __init__(self, config: Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx

        self.self_attn = InternLM3AttentionAdapter(config, layer_idx)

        self.mlp = InternLM3MLPAdapter(config)
...
def get_adapter_state_dict(model: InternLM3PreTrainedModel) -> dict[str, Any]:
    ...
    if not isinstance(model, InternLM3Model):
...
def apply_adapter(
    ...
    decoder_module: str = (
        "layers" if issubclass(model, InternLM3Model) else "model.layers"
    )

    origin_init = cast(type[InternLM3PreTrainedModel], model).__init__

    def patched_init(self: InternLM3PreTrainedModel, config: InternLM3Config):
        ...
        if (
            ...
            nested_replace_module(
                ...
                lambda _, old: nn.ModuleList(
                    [
                        InternLM3DecoderLayerAdapter(cast(Config, config), layer_idx)
